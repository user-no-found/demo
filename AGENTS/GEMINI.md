...
2. 检查是否存在 `.git/` 目录，如果不存在则先执行 `git init` 初始化 Git 仓库
3. 检查是否存在 `openspec/` 目录：
   - 如果不存在，使用 `cunzhi` 询问用户："检测到当前项目未初始化 OpenSpec，是否立即初始化？"
   - 如果用户同意，执行：`openspec init --tools gemini`
   - 初始化完成后，进入 **OpenSpec 配置引导流程**（见下方）
4. 使用 `cunzhi` 询问用户："是否将 openspec 目录加入 .gitignore？（选择是则不上传到 Git）"
5. 如果用户选择屏蔽，则将 `openspec/` 添加到 `.gitignore` 文件中
6. 如果已存在 `openspec/` 目录：
   - 读取 `openspec/specs/` 了解项目当前规范
   - 检查 `openspec/changes/` 是否有进行中的变更提案
   - 如果有进行中的提案，告知用户当前有未完成的变更
7. **检查完成后，必须通过 `cunzhi` 告知用户检查结果并等待用户指令**
...
- **禁止在git commit中添加Co-authored-by**，提交时不要添加任何AI助手的署名
...
- **禁止依赖AI训练数据中的版本信息**：使用任何第三方库、框架、工具前，必须通过 `context7` 查询最新版本和用法，AI训练数据可能已过时
...
- **当用户通过 `cunzhi` 明确表示"任务结束"、"退下"、"等待"、"等待唤醒"等结束/暂停指令时，必须立即停止所有操作，禁止继续执行任何工具调用**

记忆管理使用细节（通过 memory MCP 工具操作，禁止修改本提示词文件）：
- 对话开始时，通过 memory MCP 查询记忆，参数 `project_path` 使用当前 git 根目录的 WSL 路径格式（如 `\\wsl.localhost\Ubuntu-22.04\path\to\project`）
- 当发现用户输入"请记住："时，要对用户的消息进行总结后调用 memory MCP 的 add 功能添加记忆
- 使用 memory MCP 的 add 功能添加新记忆（content + category: rule/preference/pattern/context）
- 仅在重要变更时更新记忆，保持简洁
- 记忆全部用中文描述
- **禁止将项目路径或其他动态信息写入本提示词文件**
