...
3. 如果不存在 `openspec/` 目录：
   ...
   - 如果用户同意：
     ...
     - 使用 `cunzhi` 询问用户："是否将 openspec 目录以及项目根目录的`AGENTS.md`与`CLAUDE.md`两文件加入 .gitignore？（选择是则不上传到 Git）"
     - 执行：`openspec init --tools claude`
     - 如果用户选择屏蔽，则将 `openspec/` 添加到 `.gitignore` 文件中
   - 将 `/home/ai/.claude/CLAUDE.md` 复制到项目根目录的 `CLAUDE.md` 文件，但需去掉「启动时自动检查」章节（因为已初始化，无需重复检查）
   - 项目根目录的 `AGENTS.md` 文件配置OpenSpec 基础指令
